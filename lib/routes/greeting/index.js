...
const head = (api) => {
	const elements = [
		...
		html.title(null, api.settings.greeting.title + ' | ' + api.settings.title),
	...
	let document = '<!doctype html>' + html.html(null, [
		...
		html.body(null, [
			html.div('#page', [
				...
				html.div('.question', [
					html.h3(null, api.settings.greeting.title || ''),
...
const createGreetingRoute = (api) => (req, res, next) => {
	...
	if (!api.settings.greeting.dates.includes(day)) return next()
